...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = {
    ...
    def ascendingByRetweet0(trending: Trending, accu: TweetSet): Trending = {
      if (accu.isEmpty) trending
      else {
        ascendingByRetweet0(trending + accu.findMin, accu.remove(accu.findMin))
      }
    }
...
object GoogleVsApple {
  ...
  def filterTweets(keywords: List[String], tweets: TweetSet): TweetSet = 
    tweets.filter(t => keywords.exists(c => t.toString.contains(c)))
  
  val googleTweets: TweetSet = filterTweets(google, TweetReader.allTweets)

  val appleTweets: TweetSet = filterTweets(apple, TweetReader.allTweets)
